...
2. **Install dependencies**:
   ...
   If you see "ModuleNotFoundError: No module named 'chromadb'", install ChromaDB directly:
   ```bash
...
4. **(Optional) Enable GPT-5 mini as the default model for this project**

    Set the `OPENAI_DEFAULT_MODEL` environment variable to `gpt-5-mini` so local clients default to that model:
    ...
    - Windows (PowerShell):
       ```powershell
       $Env:OPENAI_DEFAULT_MODEL = "gpt-5-mini"
       ```

    The code will still allow overriding the model per-request (for example via the Streamlit UI).
